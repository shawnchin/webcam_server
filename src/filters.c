...
void add_text(struct image *img, char *text, struct RGB fg, struct RGB bg, struct RGB trans, int xpos, int ypos)
{
	time_t      t;
	struct tm  *tm;
	unsigned char        *line,*ptr,prepend[32];
...
//			printf("the text is too large\n");
		...
		for (x = 0; x < len; x++) {
			...
			for(i=0; i<CHAR_WIDTH; i++) {
				...
				{
					// alpha blend the old background with the new background
					// using trans variable as the alpha. trans of 0 is solid
					// new background, trans 255 is solid old backgroud.
					// everything in between is a ratio 
					ptr[0] = ((ptr[0] * trans.r) / 255)  + ((bg.r * (255-trans.r))/255);
					ptr[1] = ((ptr[1] * trans.g) / 255)  + ((bg.g * (255-trans.g))/255);
					ptr[2] = ((ptr[2] * trans.b) /255) + ((bg.b * (255-trans.b))/255);
				}
				ptr += 3;
			}
		}
